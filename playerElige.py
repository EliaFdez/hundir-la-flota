...
class Board():
    ...
    def first_board(self):
        aleatorio = input('¿Desea colocar sus propios barcos? (s/n) ')
        if (aleatorio == 's'):
            ...
            while i in range(BARCOS + 1):
                ...
                fila = input ('Fila: ')
                col  = input ('Columna: ')
                if (((int(fila) in list(range(1,ALTO + 1))))
                and ((int(col) in list(range(1,ANCHO + 1))))):
                    if tabinit[int(fila) - 1][int(col) - 1] == 1:
                        print(f"Al barquito ({fila}, {col}) ya lo echaste a nadar!")

                        i = i
                    else:
                        tabinit[int(fila) - 1][int(col) - 1] = 1
                        i += 1
                else:
                    print(f"El barquito {i} no nada en este mar")
                    i = i
            return tabinit
        elif (aleatorio == 'n'):
            print("Se los colocaremos nosotros")
            ...
            print("Elija bien otra vez:")
            return self.first_board()
